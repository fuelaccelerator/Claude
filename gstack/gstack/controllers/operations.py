...
def _get_instance_async_response(async_result, projectid, type):
    ...
    response['selfLink'] = urllib.parse.unquote_plus(
    ...
    if async_result['jobstatus'] == 0:
        response['targetLink'] = ''
        response['status'] = 'PENDING'
        response['progress'] = 0
    elif async_result['jobstatus'] == 1:
        response['status'] = 'DONE'
        response['zone'] = urllib.parse.unquote_plus(
        ...
        response['targetLink'] = urllib.parse.unquote_plus(
            helpers.get_root_url() +
            url_for(
                'getinstance',
                projectid=projectid,
